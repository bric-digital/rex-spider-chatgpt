...
function scopeConfigurationUrl(configUrlStr, scope, extensionBaseUrl) {
  ...
  for (const scopeKey of Object.keys(scope)) {
    ...
    if (scopeValue === void 0 || scopedUrl.searchParams.has(scopeKey)) {
      continue;
    }
...
var rexCorePlugin = {
  ...
  handleMessage: (message, sender, sendResponse) => {
    ...
    if (message.messageType === "setConfigurationScope") {
      rexCorePlugin.setConfigurationScope(message.scope).then((scope) => {
        sendResponse(scope);
      });
      return true;
    }
    if (message.messageType === "fetchConfigurationScope") {
      rexCorePlugin.fetchConfigurationScope().then((scope) => {
        sendResponse(scope);
      });
      return true;
    }
    if (message.messageType === "refreshConfiguration") {
      rexCorePlugin.fetchConfiguration().then((configuration) => {
        ...
        globalThis.chrome.storage.local.get(["rexIdentifier", CONFIGURATION_SCOPE_STORAGE_KEY]).then((response) => {
          const idResponse = response;
          const identifier = idResponse.rexIdentifier;
          const configurationScope = response[CONFIGURATION_SCOPE_STORAGE_KEY] ?? {};
          const configUrl = scopeConfigurationUrl(resolveConfigurationUrl(configUrlStr, identifier), configurationScope, globalThis.chrome.runtime.getURL("/"));
  ...
  fetchConfigurationScope: () => {
    return globalThis.chrome.storage.local.get(CONFIGURATION_SCOPE_STORAGE_KEY).then((response) => {
      const storedScope = response[CONFIGURATION_SCOPE_STORAGE_KEY];
      return storedScope ?? {};
    });
  },
  setConfigurationScope: (scope) => {
    return rexCorePlugin.fetchConfigurationScope().then((currentScope) => {
      const updatedScope = { ...currentScope };
      for (const scopeKey of Object.keys(scope ?? {})) {
        const scopeValue = scope[scopeKey];
        if (scopeValue === null || scopeValue === void 0) {
          delete updatedScope[scopeKey];
        } else if (typeof scopeValue === "string") {
          updatedScope[scopeKey] = scopeValue;
        }
      }
      return globalThis.chrome.storage.local.set({ [CONFIGURATION_SCOPE_STORAGE_KEY]: updatedScope }).then(() => updatedScope);
    });
  }
};
...
var REXSpider = class {
  constructor() {
    ...
    __publicField(this, "startCrawl", null);
    __publicField(this, "endCrawl", null);
    __publicField(this, "haltOnError", true);
    __publicField(this, "timeAnchor", "runtime");
    __publicField(this, "crawlDelay", 3e4);
    // 30 seconds, by default - may be configured
    __publicField(this, "sleepDelay", 3e5);
    // 5 minutes, by default - may be configured
    __publicField(this, "recrawlDelay", 0);
    __publicField(this, "summarize", false);
    __publicField(this, "crawling", false);
  }
  updateConfiguration(configuration) {
    if (configuration.enabled !== void 0) {
      this.enabled = configuration.enabled;
    }
    if (configuration.start !== void 0) {
      this.startCrawl = configuration.start;
    }
    if (configuration.end !== void 0) {
      this.endCrawl = configuration.end;
    }
    if (configuration["time_anchor"] !== void 0) {
      this.timeAnchor = configuration["time_anchor"];
    }
    if (configuration["crawl_delay"] !== void 0) {
      this.crawlDelay = configuration["crawl_delay"];
    }
    if (configuration["sleep_delay"] !== void 0) {
  ...
  prepareCrawl() {
    ...
    service_worker_default.handleMessage(storeMessage, this, (response) => {
      this.crawling = true;
      this.recrawlDelay = 0;
    });
  }
  sleepElapsed() {
    return new Promise((resolve) => {
      ...
      service_worker_default.handleMessage(fetchLastCrawl, this, (response) => {
  ...
  crawlWindowContains(timestamp) {
    return new Promise((resolve) => {
      this.crawlWindowStart().then((startTime) => {
        if (startTime !== null && timestamp < startTime) {
          resolve(false);
        } else {
          this.crawlWindowEnd().then((endTime) => {
  ...
  signalAccountCollectionComplete(details = {}) {
    dispatchEvent({
      name: "pdk-app-event",
  ...
  signalCrawlComplete(crawledCount, crawledIds = [], reason = "None given", clearFlag = true) {
    dispatchEvent({
      name: "pdk-app-event",
  ...
  fetchUploadKey(convoId, updated) {
    const timestamp = Math.round(updated.timestamp());
    return `rex-spider-${this.identifier()}-conversation-upload-${convoId}-${timestamp}`;
  }
  checkIfAlreadyTransmitted(identifier, updated) {
    ...
    return new Promise((resolve) => {
      ...
      service_worker_default.handleMessage(fetchTransmission, this, (response) => {
        if (response !== null) {
          resolve(true);
        } else {
          resolve(false);
  ...
  logTransmitted(identifier, updated) {
    const uploadKey = this.fetchUploadKey(identifier, updated);
    return new Promise((resolve) => {
      const logTimestamp = {
        messageType: "storeValue",
      ...
      service_worker_default.handleMessage(logTimestamp, this, (response) => {
        resolve();
      });
    });
  }
...
var REXSpiderModule = class extends REXServiceWorkerModule {
  ...
  updateConfiguration(config) {
    ...
    if (spiderConfigs !== void 0) {
      const defaultConfig = spiderConfigs["default"];
      for (const spider of this.registeredSpiders) {
        const spiderConfig = spiderConfigs[spider.identifier()];
        if (spiderConfig !== void 0) {
          spider.updateConfiguration(spiderConfig);
        } else if (defaultConfig !== void 0) {
  ...
  handleMessage(message, sender, sendResponse) {
    if (message.messageType == "beginBackgroundCrawls") {
      ...
      const spidersToCrawl = new REXStackOperator();
      for (const spider of this.registeredSpiders) {
      ...
      spidersToCrawl.run((spider) => {
        return new Promise((spiderResolve) => {
          if (spider.isCrawling()) {
            ...
            spider.sleepElapsed().then((elapsed) => {
              if (elapsed) {
                console.log(`[rex-spider: ${spider.identifier()}] Starting crawl ${/* @__PURE__ */ new Date()}...`);
                spider.doBackgroundCrawl().then((result) => {
...
var import_check_types2 = __toESM(require_check_types(), 1);
var REXChatGPTSpider = class extends REXSpider {
  constructor() {
    super(...arguments);
        ...
        if (turnId !== void 0) {
          ...
          if (turnJson !== void 0) {
            ...
            if (turnJson.message !== null) {
              ...
              if (messageCreateTime !== null && messageCreateTime !== void 0) {
                createTime = new DateString(messageCreateTime);
                const turnDate = new Date(messageCreateTime * 1e3);
              ...
              if (turnJson.message.content.parts !== void 0) {
                ...
                turn["content*"] = turnJson.message.content.thoughts.map((thought) => {
                  const summary = typeof thought?.summary === "string" ? thought.summary : "";
                  const content = typeof thought?.content === "string" ? thought.content : "";
                  return [summary, content].filter((s2) => s2.length > 0).join("\n");
                }).filter((s2) => s2.length > 0).join("\n\n");
          ...
          if (!response.ok) {
            ...
            response.json().then((body) => {
              if (body.items !== void 0) {
                const toProcess = [];
                if (import_check_types2.default.array(body.items) && body.items.length === 0) {
                  ...
                  const processNextItem = () => {
                    if (toProcess.length == 0) {
                      ...
                      if (item !== void 0) {
                        if (item["update_time"] !== void 0) {
                          try {
                            const updated = new DateString(item["update_time"]);
                            const created = new DateString(item["create_time"]);
                            const title = item.title;
                            if (item.id !== void 0) {
                              this.crawlWindowContains(updated.timestamp()).then((isContained) => {
                                if (isContained) {
                                  inCrawlWindowCount += 1;
                                  this.checkIfAlreadyTransmitted(item.id, updated).then((transmitted) => {
  ...
  fetchConversationRecordsForProject(projectId) {
    return new Promise((resolve, reject) => {
      const inspectionRecords = [];
      const fetchProjectConversationsForPage = (cursorToken = "0") => {
        if (cursorToken === void 0 || cursorToken === null) {
          resolve(inspectionRecords);
        } else {
          const projectUrlBase = `https://chatgpt.com/backend-api/gizmos/${projectId}/conversations`;
          ...
          if (parsedUrl !== null) {
            ...
            fetch(parsedUrl.href, {
              ...
              if (response.ok) {
                response.json().then((body) => {
                  if (import_check_types2.default.array(body.items)) {
                    if (body.items.length === 0) {
                      ...
                      const checkNextItem = () => {
                        if (toCheck.length == 0) {
                          ...
                          if (import_check_types2.default.string(item.id) && inspectionRecords.includes(item.id) === false) {
                            ...
                            if (Number.isNaN(updated)) {
                              ...
                              this.crawlWindowContains(updated).then((include) => {
                                if (include) {
                                  const updatedString = new DateString(updated / 1e3);
                                  const startedString = new DateString(started / 1e3);
                                  this.checkIfAlreadyTransmitted(item.id, updatedString).then((transmitted) => {
                                    if (transmitted === false) {
                                      inspectionRecords.push({
                                        id: item.id,
                                        refresh: true,
  ...
  fetchProjectRecords() {
    return new Promise((resolve, reject) => {
      ...
      const fetchSidebarIds = (cursorToken = null) => {
        const sidebarUrlBase = `https://chatgpt.com/backend-api/gizmos/snorlax/sidebar?owned_only=true&conversations_per_gizmo=0&limit=${this.pageSize}`;
                ...
                if (body.items !== void 0) {
                  ...
                  const checkNextProject = () => {
                    if (toCheck.length === 0) {
                      ...
                      if (import_check_types2.default.string(gizmoId) && gizmoId.startsWith("g-p-")) {
                        this.fetchConversationRecordsForProject(gizmoId).then((projectRecords) => {
                          if (import_check_types2.default.array(projectRecords)) {
  ...
  doBackgroundCrawl() {
    return new Promise((resolve) => {
      super.doBackgroundCrawl().then((crawlResult) => {
        const crawledIds = [];
              ...
              if (this.accessToken === null) {
                ...
                const fetchNextConversation = () => {
                  if (toCrawl.length === 0) {
                    ...
                    const convoRecord = toCrawl.pop();
                    if (convoRecord !== void 0) {
                      if (crawledIds.includes(convoRecord.id) === false) {
                        crawledIds.push(convoRecord.id);
                      }
                      if (convoRecord.refresh) {
                        if (this.justSummarize()) {
                          this.checkIfAlreadyTransmitted(convoRecord.id, convoRecord.lookupDate).then((transmitted) => {
                            const conversation = {
                              turns: [],
                              platform: "chatgpt",
                ...
                setTimeout(() => {
                  this.fetchConversationRecords().then((convoIds) => {
                    ...
                    setTimeout(() => {
                      this.fetchProjectRecords().then((projectRecords) => {
...
var chatGPTSpider = new REXChatGPTSpider();
service_worker_default2.registerSpider(chatGPTSpider);
var service_worker_default3 = chatGPTSpider;
...
self["rexChatGPTPlugin"] = service_worker_default3;
